...
START_TEST(test_key_one_element_array)
{
    ...
    int *pa_dst = malloc(sizeof(int));

    rc = key(pa_src, pb_src, &pa_dst, 1, 1);
    free(pa_dst);
...
START_TEST(test_key_reversed_array)
{
    ...
    int *pa_dst = malloc((pb_src - pa_src) * sizeof(int)), *pb_dst = NULL;
    size_t cur_size;

    int result_array[] = {4, 3, 2};
    int *pa = result_array, *pb = pa + 3;

    rc = key(pa_src, pb_src, &pa_dst, pb_src - pa_src, &cur_size);
