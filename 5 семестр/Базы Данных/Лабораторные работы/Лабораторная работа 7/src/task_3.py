...
class Lawyer(BaseModel):
    ...
    email_address = CharField(column_name='email_address')
    phone_number = CharField(column_name='phone_number')
    skill = IntegerField(column_name='skill')
    office_address = CharField(column_name='office_address')
    process_count = IntegerField(column_name='process_count')
...
def query_1():
    ...
    lawyer = Lawyer.get(Lawyer.id == 2)
    print(GREEN, f'{"1. Однотабличный запрос на выборку:":^130}')
    print(lawyer.id, lawyer.name, lawyer.email_address, lawyer.phone_number, lawyer.skill, lawyer.office_address, lawyer.process_count)

    # Получаем набор записей.
    query = Lawyer.select().where(Lawyer.skill > 5).limit(5).order_by(Lawyer.id)

    print(BLUE, f'\n{"Запрос:":^130}\n\n', query, '\n')

    lawyers_selected = query.dicts().execute()

    print(YELLOW, f'\n{"Результат:":^130}\n')
    for elem in lawyers_selected:
        print(elem)
...
#     query = Users.select(Users.id, Users.nickname, World.name).join(BlackList).join(World).order_by(Users.id).limit(
#         5).where(Users.id > 2)
#
...
#             Users.create(id=new_id, nickname=new_nickname, age=new_age, sex=new_sex,
#                          number_of_hours=new_number_of_hours, id_device=new_id_device)
